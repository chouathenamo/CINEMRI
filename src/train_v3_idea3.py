"""
train_v3_idea3.py — Idea 3 variant (small ODE + mean-pool trajectory + residual)
==================================================================================
Changes vs train_v3_residual.py:
  - imports from model_v3_idea3
  - pat=N/30 added to per-epoch print so patience is visible in logs

Run:
    mkdir -p logs && nohup python train_v3_idea3.py \\
        ...
        --results_file results_idea3_p30.json \\
        --wandb_project cardiac-ode --wandb_run_name idea3_p30 \\
        > logs/idea3_p30.txt 2>&1 &
    echo "PID: $!"
    tail -f logs/idea3_p30.txt
"""
...
from dataset_v3 import (ACDCSliceDataset, collate_fn,
                         ...
                         N_CLINICAL_FEATURES)
from model_v3_idea3 import CardiacMotionODE, ModelOutput          # ← only import change
...
def stratified_kfold(march9_dir, training_dir, patient_ids, args, device, n_splits=5):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(patient_ids, labels_all)):
        ...
        # Print ODE parameter count so we can verify it's small
        ode_params = sum(p.numel() for p in model.ode_classifier.ode_block.parameters())
        _flush(f"  ODE parameters: {ode_params:,}  (idea3: should be ~4k)")
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            # ── patience counter now printed in log ───────────────────────────
            _flush(
                ...
                f"pat={patience_ctr}/{args.patience} "   # ← visible patience
                f"({elapsed:.0f}s)"
            )
...
def parse_args():
    p = argparse.ArgumentParser(description="Train CardiacMotionODE (idea3 variant)")
    ...
    p.add_argument("--results_file",      type=str, default="results_idea3.json")
...
def main():
    ...
    _flush(f"Model variant: idea3 (small ODE d=32 + mean-pool trajectory + residual)")

    results = stratified_kfold(
        args.march9_dir, args.training_dir, all_pids, args, device, args.n_splits)

    results["config"]   = vars(args)
    results["ablation"] = "idea3_ode"
