"""
ode.py — Neural ODE for Cardiac Contraction Dynamics  (v3)
============================================================
Changes from v2:
  - ODEFunc dropout 0.1 → 0.3 (regularisation)
  - ClassifierHead:
      · accepts optional clinical features concatenated to z_final
      · dropout 0.3 → 0.5
      · simplified to single hidden layer (256) → fewer parameters
  - CardiacODEClassifier:
      · new n_clinical parameter (default 8, set to 0 to disable)
      · forward() accepts optional clinical_feat (B, n_clinical) tensor
"""
...
class ClassifierHead(nn.Module):
    """
    MLP from z_final (optionally concatenated with clinical features) → logits.

    v3 changes:
      - in_dim = d_z + n_clinical  (was always d_z)
      - dropout 0.3 → 0.5
      - simplified: Linear(in_dim, 256) → ReLU → Dropout → Linear(256, n_classes)
    """
...
class CardiacODEClassifier(nn.Module):
    ...
    v3: accepts optional clinical_feat (B, n_clinical) that is
    concatenated to z_final before the classifier head.
    """
    ...
    def forward(
        ...
        """
        z0 = self.seq_encoder(embeddings)
        ...
        # Concatenate clinical features if provided
        if clinical_feat is not None and self.n_clinical > 0:
            cf = clinical_feat.to(z_final.device)
            z_in = torch.cat([z_final, cf], dim=1)  # (B, d_z + n_clinical)
        else:
            # Pad with zeros if clinical features not available
...
if __name__ == "__main__":
    ...
    print("[ode_v3.py] All checks passed.")
